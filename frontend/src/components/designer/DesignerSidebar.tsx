import { LayoutDashboard, Palette, Wallet, TrendingUp, LogOut, Menu } from 'lucide-react';
import { NavLink as RouterNavLink, useLocation } from 'react-router-dom';
import { Button } from '@/components/ui/button';
import { useState } from 'react';
import { cn } from '@/lib/utils';
import { currentDesigner } from '@/data/adminData';

const menuItems = [
  { title: 'Dashboard', url: '/designer', icon: LayoutDashboard },
  { title: 'My Designs', url: '/designer/designs', icon: Palette },
  { title: 'Sales', url: '/designer/sales', icon: TrendingUp },
          ...
          {!collapsed && (
            <div className="p-4 border-b border-border">
              <div className="flex items-center gap-3">
                <img 
                  src={currentDesigner.avatar} 
                  alt={currentDesigner.name}
                  className="w-10 h-10 rounded-full object-cover"
                />
                <div>
                  <p className="font-medium text-sm">{currentDesigner.name}</p>
                  <p className="text-xs text-muted-foreground">{currentDesigner.email}</p>
                </div>
              </div>
            </div>
          )}
